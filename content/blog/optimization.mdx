...
In other words, model complexity (parameter count $N$) and task complexity (effective data dimension $m$) elegantly decouple. Increasing parameters without fundamentally new data geometries merely increases redundancy (the bulk swells), but the set of outliers remains effectively bounded by the intrinsic dimension of the data.
...
Subsequent work has revealed an even richer picture. These "valleys" of minima are not merely connected pairwise but coalesce into complex, hierarchical networks (Draxler et al., 2018) <Citation author="Draxler et al." year="2018" title="Essentially No Barriers in Neural Network Energy Landscape" url="https://arxiv.org/abs/1803.00885   " venue="ICML"/>. The loss landscape is best conceived not as a set of separate basins, but as an intricate web where high-dimensional fibres merge and branch, forming a veritable manifold of manifolds.
...
This geometric view recasts the entire scaling narrative. We've been conflating computational tractability (the blessing of high-dimensional optimization) with something far more fundamental: the expansion of the data manifold's intrinsic geometry. The former merely enables navigation; the latter defines the territory worth navigating.
...
The endless proliferation of optimizer variants—AdaGrad, RMSprop, AdamW, LAMB, Muon—represents exploration of an already-mapped territory. These methods offer engineering advantages (memory efficiency, parallelization, numerical stability) but not fundamental improvements. Similarly, architecture search might accelerate training but won't unlock new capabilities at sufficient scale.
...
The deeper lesson: in the over-parameterized regime, optimization algorithms become implementation details. The fundamental objects are the data distribution (defining the task), the model capacity (defining the fiber dimension), and the compute budget (defining how thoroughly we explore the fiber). Everything else—optimizer choice, architecture details, initialization schemes—merely affects the engineering question of how quickly we converge to the inevitable destination.
