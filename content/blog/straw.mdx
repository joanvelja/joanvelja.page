...
image: '/images/blog/straw/straw2.png'
date: '2025-12-01'
...
$$G_\ell = \sum_{t=1}^T (R-b_t) u_{\ell,t} x_{\ell,t}^\top,  \tag{1}$$
...
Consequently, if $$t\notin\mathcal D$$ the RHS is $$0$$ (no signal in expectation). If $$t\in\mathcal D$$, define
 $$\mu_1(t):=\mathbb E[R\mid \tau_{\neg t},A_t\in S_t(Y)]$$, $$\mu_0(t):=\mathbb E[R\mid \tau_{\neg t},A_t\notin S_t(Y)]$$; with Assumptions 2–3,
$$\boxed{\mathbb E\big[(R-b_t) u_{L,t}\bigm|\tau_{\neg t}\big]=\big(\mu_1(t)-\mu_0(t)\big)\Big((p_t \circ \mathbf 1_{S_t})-q_t p_t\Big),}\tag{3}$$

which lies in $$\operatorname{span}\{e_s-p_t:\ s\in S_t(Y)\}$$ (dimension $$\le |S_t(Y)|$$). In the singleton case $$S_t(Y)=\{y\}$$,
$$\boxed{\mathbb E \big[(R-b_t) u_{L,t}\bigm|\tau_{\neg t}\big]=\big(\mu_1(t)-\mu_0(t)\big) p_t[y] (e_y-p_t),}\tag{4}$$
...
$$G_{\text{out}}=\sum_{t=1}^T (R-b_t),u_{L,t},h_t^\top.\tag{5}$$
